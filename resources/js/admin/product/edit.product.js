...
existingImagesContainer.addEventListener('click', function(e) {
    if (e.target.classList.contains('remove-existing')) {
        ...
        // Xóa luôn input ẩn để backend biết ảnh bị xóa
        wrapper.querySelector('input[name="existing_images[]"]')?.remove();
        wrapper.remove();
    }
});

// ------------------------
// QUẢN LÝ ẢNH MỚI (UPLOAD)
// ------------------------
const imagesInput = document.getElementById('images');
const previewImages = document.getElementById('previewImages');
const dt = new DataTransfer(); // Lưu các file mới được chọn
...
function renderPreview() {
    ...
    Array.from(dt.files).forEach((file, index) => {
        ...
        reader.onload = function (e) {
            ...
            // Xóa file khỏi DataTransfer khi nhấn nút
    ...
    // Gán lại file vào input để submit
    imagesInput.files = dt.files;
}
